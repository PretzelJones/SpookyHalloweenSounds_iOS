...
final class HiddenViewController: UIHostingController<HiddenView> {
    ...
    required init?(coder: NSCoder) {
        super.init(coder: coder, rootView: HiddenView())
    }

    override func viewDidLoad() {
        super.viewDidLoad()

        let titleHC = UIHostingController(rootView: PaletteText(text: "Hidden Sounds", font: .custom("Creepster", size: 28)))
